...
All strategies below are implemented in `sorry-core`. Each exposes its name, one-line description, and a complexity tier (`Trivial` / `Low` / `Medium`) via its `describe()` impl, which `/rules` and `/simulate` read through `get_strategy_descriptions()`. Keep this section authoritative for the full specs so `/rules` can stay brief. All strategies receive a `StrategyView` with all public knowledge (hand, pawn positions for all players, discard contents, deck size) and must route non-determinism through the provided `rng`.

- **Random** *(Trivial)* — picks a legal move uniformly at random. Baseline.

- **Greedy** *(Low)* — always minimizes distance-to-home. Given multiple moves, picks the one that brings the average (or best) pawn closest to its home space. Ties broken by preferring safety-zone spaces.

- **Not Sorry** *(Low)* — maximizes pawns out of Start. Any move that starts a pawn (1, 2, Sorry!) is taken whenever possible; otherwise plays as **Greedy**.

- **Teleporter** *(Low)* — **always** takes an 11-swap when legal, even if the swap moves its own pawn backwards along the track. When multiple swap targets exist, picks the one that maximizes its own distance around the board relative to its `start_exit`. Falls back to **Greedy** when no swap is available.

- **Survivor** *(Medium)* — attacks the current leader (smallest average distance-to-home among opponents) regardless of cost to its own position. Decision order:
...
- **Reverse** *(Medium)* — holds one primary pawn near its `start_exit`, hoping for a 4 (15-space forward-equivalent) or a chain of 10s to reach Home quickly. Secondary pawns stay in Start unless forced. Counts discard to estimate remaining 4s / 10s before committing the primary pawn forward.

- **Sidekick** *(Medium)* — piles on whichever opponent most recently lost a pawn (had one bumped to Start). When no recent victim exists, plays as **Greedy**.

- **Loser** *(Medium)* — deliberately plays to lose. Prefers moves that avoid bumping opponents back to Start, then maximizes its own distance from Home (worst legal move). Useful as a control for showing that the other strategies are actually trying to win.
